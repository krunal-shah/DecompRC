...
CUDA_VISIBLE_DEVICES=x,y bash aggregate.sh {ARG}
```
where `{ARG}` is one of 
...
**Note**: At a time, only one perturbation can be run on a system since all the experiments use and modify similar files.
