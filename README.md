...
2. Download the HotpotQA data from [link](https://hotpotqa.github.io) and the pretrained BERT model from [link](https://drive.google.com/file/d/1XaMX-u5ZkWGH3f0gPrDtrBK1lKDU-QFk/view). Place the dataset files in a directory `dataset` and place the downloaded model in a directory `model`.<br>
3. Postprocess the HotpotQA data using <br>
```python convert_hotpot2squad.py --data_dir {DATA_DIR} --task hotpot-all```
`{DATA_DIR}` is the directory containing the HotpotQA dataset files.
4. Download the data and models from [link](https://drive.google.com/file/d/1p7VrJIEmUY9tAWmx31chhStS-KoRWOJQ/view?usp=sharing), unzip it and place the contents in the root directory.

The final directory structure is expected to look like
```
    .
    ├── model/                            # from steps 2 and 4
    ├── data/                             # from steps 3 (output of the command) and 4
    ├── dataset/                          # from step 2
    ├── other files from this repository  # from step 1
```
### Inference
```
CUDA_VISIBLE_DEVICES=x,y bash bridge.sh {ARG}
CUDA_VISIBLE_DEVICES=x,y bash intersection.sh {ARG}
CUDA_VISIBLE_DEVICES=x,y bash aggregate.sh
python3 show_result.py --data_file {HOTPOT_DEV_FILE} --prediction_file {FILE_TO_SAVE}
```
where `{ARG}` is one of 
